...
class T03_Base_Rights_H_Data2Dw(Bybo_base):
    def __init__(self):
        super().__init__()
    ...
    def Draw_data(self,end_date,begin_date):
        ...
        try:
            ...
            sql2Tmp = '''
                        insert into T03_Base_Rights_H_Tmp
                    ...
                    ,SourceName)
                    select 
                     ifnull(a.id,'') as RightsId
                    ,ifnull(a.code,'') as RightsCode
                    ,ifnull(a.rights_type,'0') as RightsType
                    ,ifnull(a.selling_price,'0') as SellingPrice
                    ,ifnull(a.settlement_price,'0') as SettlementPrice
                    ,ifnull(a.base,'0') as BasePrice
                    ,ifnull(a.quota,'') as QuotaPrice
                    ,ifnull(a.platform,'') as Platform
                    ,ifnull(a.is_availability,'0') as Is_Availability
                    ...
                    ,md5(CONCAT(a.id,'_',a.updated_time))                     as Contrast_Id
                    ,'bbkq_ods_card.base_rights'                  as SourceName
                    from %s.base_rights a
                    where  ifnull(a.updated_time,a.Created_time)
                    >=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
                    and ifnull(a.updated_time,a.Created_time)
                    <=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
           ''' % (ods_card,begin_date, end_date)
            print(sql2Tmp)
            cur.execute(sql2Tmp)
            ...
            sql2Log = '''
                       insert into T03_Base_Rights_H_Log
                        ( RightsId
                        ,RightsCode
                        ,RightsType
                        ,SellingPrice
                        ...
                        ,a.Contrast_Id
                        ,a.SourceName
                        from T03_Base_Rights_H_Tmp a
                        left join T03_Base_Rights_H b  on a.Contrast_Id = b.Contrast_Id and b.End_date='2099-01-01 00:00:00'
            ...
            sql_P2_Update = '''
            update T03_Base_Rights_H a inner join  T03_Base_Rights_H_log b  on a.Contrast_Id = b.Contrast_Id
          ...
          '''% (begin_date, end_date)
            ...
            self.addMessage(conn, 'T03_Base_Rights_H_Data2Dw.py ', jobMsg, jobSt, starttime.strftime(stF),
...
if __name__ == '__main__':
    ...
    try:
        ...
        F_or_N = eg.get_rowcount('T03_Base_Rights_H')
        print(F_or_N)
        if F_or_N > 0:
