...
class T03_Template_Org_H_Data2Dw(Bybo_base):
    def __init__(self):
        super().__init__()
        self.__name = 'T03_Template_Org_H'
        self._taget_table = 'T03_Template_Org_H'
    ...
    def Draw_data(self,end_date,begin_date):
        ...
        try:
            ...
            sql2Tmp = '''
                    ...
                    ,SourceName)
                    select 
                    ifnull(a.TemplateId,'') as TemplateId
                    ,concat(c.name,LPAD(b.officeid, 10, 0))  as OfficeId
                    ,ifnull(a.IsAvailability,'') as IsAvailability
                    ...
                    md5(a.TemplateId)                     as Contrast_Id,
                    'bbkq_ods_card.template'          as SourceName
                    from (
                    select  
                    ...
                    ,a.id as TemplateId
                    ,SUBSTRING_INDEX( SUBSTRING_INDEX( a.apply_org, ',', d.help_topic_id + 1 ), ',',- 1 )  as OfficeId
                    ,a.is_availability as IsAvailability
                    ,a.platform as Platform
                    from %s.template a
                    JOIN mysql.help_topic d ON d.help_topic_id < ( LENGTH( a.apply_org ) - LENGTH( REPLACE ( a.apply_org, ',', '' ) ) + 1 )) a
                    left join %s.office b on a.OfficeId=b.id
                    left join bbkq_dw.t_code c on b.TenantId=c.code
                    where c.codetype='Region'
                    and  ifnull(a.updated_time,a.created_time)
                    >=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
                    and ifnull(a.updated_time,a.created_time)
                    <=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
           ''' % (self.__name,ods_card,ods_card,begin_date, end_date)
            print(sql2Tmp)
            cur.execute(sql2Tmp)
...
if __name__ == '__main__':
    ...
    try:
        ...
        F_or_N = eg.get_rowcount('T03_Template_Org_H')
        print(F_or_N)
        if F_or_N > 0:
