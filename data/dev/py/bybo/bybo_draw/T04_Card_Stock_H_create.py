...
class T04_Card_Stock_H_Createtable(Bybo_base):
    def __init__(self):
        super().__init__()
        self.__name='T04_Card_Stock_H'
        self._taget_table = 'T04_Card_Stock_H'
    ...
    (
            StockId varchar(50) NOT NULL    DEFAULT ''COMMENT'库存编号',
            TemplateId varchar(50) NOT NULL    DEFAULT ''COMMENT'卡券模版编号',
            SettlementOrgId varchar(50) NOT NULL    DEFAULT ''COMMENT'内部结算主体编号',
            StockType varchar(10) NOT NULL    DEFAULT ''COMMENT'库存类型代码',
            PlatForm varchar(10) NOT NULL    DEFAULT ''COMMENT'平台代码',
            IsEnable varchar(10) NOT NULL    DEFAULT ''COMMENT'启动标识',
            IsAvailability varchar(10) NOT NULL    DEFAULT ''COMMENT'有效标识',
            StockAvailableNumber int NOT NULL    DEFAULT '0'COMMENT'可用库存数量',
            StockTotalAmount int NOT NULL    DEFAULT '0'COMMENT'累积库存数量',
            IncreaseAmount int NOT NULL    DEFAULT '0'COMMENT'自动添加库存的数量',
  ...
  DEFAULT CHARSET = utf8;
            ...
            self.addMessage(conn, 'T01_Outter_Stock_H.py', jobMsg, jobSt, starttime.strftime(stF),
...
if __name__ == '__main__':
    eg =T04_Card_Stock_H_Createtable()
    logger = eg.logger
    logger.info("任务开始")
    stF = "%Y-%m-%d %H:%M:%S"
    # 默认为前一天
