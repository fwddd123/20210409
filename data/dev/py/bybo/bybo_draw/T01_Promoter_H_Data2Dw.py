...
class T01_Promoter_H_Data2Dw(Bybo_base):
    def __init__(self):
        super().__init__()
    ...
    def Draw_data(self,end_date,begin_date):
        ...
        try:
            ...
            sql2Tmp = '''
                ...
                ,SourceName)
                SELECT 
                ifnull(a.new_promoterid,'') as PromoterId
                ,ifnull(a.new_code,'') as PromoterCode
                ,ifnull(a.new_businessunit_id,'') as BusinessUnitId
                ,ifnull(a.new_channel_id,'') as ChannelId
                ,ifnull(concat(b.Region,LPAD(a.new_epromoterid, 10, 0)),'') as ProviderId
                ,ifnull(a.new_stores,'') as StoresId
                ,ifnull(a.new_departmenttype,'') as DepartmentTypeCode
                ,ifnull(a.new_name,'') as PromoterName
                ,ifnull(DATE_FORMAT(a.createdon,'%%Y-%%m-%%d'),'1900-01-01') as CreatedOnDate
                ,ifnull(a.createdon,'') as CreatedOnTime
                ,ifnull(a.modifiedon,a.createdon) as ModifiedOnTime
                ,ifnull(concat(b.Region,LPAD(b.officeid, 10, 0)),'') as OfficeId
                ...
                md5(ifnull(a.new_promoterid,''))                     as Contrast_Id,
                'bbkq_ods_crm.NEW_PROMOTERBASE'          as SourceName
                FROM %s.new_promoterbase a
                left join %s.provider  b on a.new_epromoterid=b.id	
                where 
                ifnull(a.ModifiedOn,a.CreatedOn)
                >=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
                and ifnull(a.ModifiedOn,CreatedOn)
                <=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
           ''' % (self.__name, ods_crm, ods_ekanya, begin_date, end_date)
            print(sql2Tmp)
            cur.execute(sql2Tmp)
...
if __name__ == '__main__':
    ...
    try:
        ...
        F_or_N = eg.get_rowcount('T01_Promoter_H')
        print(F_or_N)
        if F_or_N > 0:
