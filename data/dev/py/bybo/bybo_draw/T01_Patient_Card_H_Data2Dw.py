...
class T01_Patient_Card_H_Data2Dw(Bybo_base):
    def __init__(self):
        super().__init__()
    ...
    def Draw_data(self,end_date,begin_date):
        ...
        try:
            ...
            sql2Tmp = '''
                ...
                ,ifnull(a.new_external_id,'') as ExternalId
                ,ifnull(a.new_external_order_id,'') as ExternalOrderId
                ,ifnull(a.new_my_cardId,'') as CardId
                ,ifnull(a.new_card_no,'') as CardNo
                ,ifnull(a.new_transactor,'') as TransactorID
                ,ifnull(a.new_transactorName,'') as TransactorName
                ,ifnull(a.new_card_channelsName,'') as CardChannelsName
                ,ifnull(a.new_state,'') as State
                ,ifnull(a.new_external_price,'0') as ExternalPrice
                ,ifnull(DATE_FORMAT(a.CreatedOn,'%%Y-%%m-%%d'),'') as CreatedOnDate
                ,ifnull(a.CreatedOn,'') as CreatedOnTime,
                ifnull(a.new_is_availability,'') as IsInactive
                ,ifnull(a.statecode,'') as Statecode
                ,ifnull(a.statuscode,'') as Statuscode,
                 ifnull(a.CreatedOn, '') as RecordCreatedTime,
                ifnull(a.ModifiedOn, a.CreatedOn)     as RecordUpdatedTime,
                ifnull(a.CreatedOn, '1900-00-00 00:00:00')   as Start_date,
                now()                                              as InDw_timestamp,
                '1'        as Partitions_ID, # 用时间做分区键
                md5(ifnull(a.new_card_no,''))                     as Contrast_Id,
                'bbkq_ods_crm.new_my_card'          as SourceName
                from %s.NEW_MY_CARD a
                where 
                ifnull(a.ModifiedOn,a.CreatedOn)
                >=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
                and ifnull(a.ModifiedOn,CreatedOn)
                <=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
           ''' % (self.__name, ods_crm, begin_date, end_date)
            print(sql2Tmp)
            cur.execute(sql2Tmp)
...
if __name__ == '__main__':
    ...
    end_date2020_t2 = '2021-06-14 23:59:59'
    ...
    try:
        ...
        F_or_N = eg.get_rowcount('T01_Patient_Card_H')
        print(F_or_N)
        if F_or_N > 0:
