...
class T02_Rights_hxRecord_H_Data2Dw(Bybo_base):
    def __init__(self):
        super().__init__()
    ...
    def Draw_data(self,end_date,begin_date):
        ...
        try:
            ...
            sql_P1_Del = '''
                        delete from T02_Rights_hxRecord_H
            ...
            sql2Tmp = '''
                         insert into T02_Rights_hxRecord_H_Tmp
            ...
            select 
             ...
             ifnull(a.new_my_rights_hxrecordid,'') as RightsHXRecordId
            ,ifnull(a.new_account_id,'') as AccountId
            ,ifnull(a.new_bill_id,'') as ChargeOrderId
            ,ifnull(a.new_my_card_id,'') as CardId
            ,ifnull(a.new_rights_type,'') as RightsTypeId
            ,ifnull(DATE_FORMAT(a.CreatedOn,'%%Y-%%m-%%d'),'') as CreatedOnDateTime
            ,ifnull(a.CreatedOn,'1900-00-00 00:00:00') as CreatedOnDate
            ,ifnull(a.statecode,'') as State
            ,ifnull(b.name,'') as Region,
            ifnull(a.ModifiedBy,'')                    as RecordCreatedUser,
            ifnull(a.CreatedOn, '') as RecordCreatedTime,
            ifnull(a.ModifiedOn, a.CreatedOn)     as RecordUpdatedTime,
            ifnull(a.ModifiedOn, '1900-00-00 00:00:00')   as Start_date,
            now()                                              as InDw_timestamp,
            '1'        as Partitions_ID, # 用时间做分区键
            md5(a.new_my_rights_hxrecordId)                     as Contrast_Id,
            'bbkq_ods_ekanya.new_my_rights_hxrecord'                  as SourceName
             from %s.new_my_rights_hxrecord a
            left join t_Code b on a.new_tenant_id =b.code
      
           where  ifnull(a.ModifiedOn,a.CreatedOn)
           >=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
           and ifnull(a.ModifiedOn,a.CreatedOn)
           <=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
           ''' % (ods_crm,begin_date, end_date)
            print(sql2Tmp)
            cur.execute(sql2Tmp)

            sqlDel = '''
                        delete from T02_Rights_hxRecord_H_Log
                         where Start_date >= str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
                         and Start_date <= str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
                                                                        ''' % (begin_date, end_date)
            print(sqlDel)
            ...
            sql2Log = '''
                        insert into T02_Rights_hxRecord_H_Log
                        (RightsHXRecordId
                        ,AccountId
                        ,ChargeOrderId
                        ,CardId
                        ...
                        ,a.Contrast_Id
                        ,a.SourceName
                         from T02_Rights_hxRecord_H_Tmp a
                         left join T02_Rights_hxRecord_H b  on a.Contrast_Id = b.Contrast_Id and b.End_date='2099-01-01 00:00:00'
            ...
            sql_P2_Update = '''
            update T02_Rights_hxRecord_H a inner join  T02_Rights_hxRecord_H_log b  on a.Contrast_Id = b.Contrast_Id
...
and B.Start_date >= str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
            ...
            sql_P2_Write = '''
             insert into T02_Rights_hxRecord_H																		
                	(RightsHXRecordId
                    ,AccountId
                    ,ChargeOrderId
                    ,CardId
                ...
                ,a.Contrast_Id
                ,a.SourceName
                from T02_Rights_hxRecord_H_log a
...
if __name__ == '__main__':
    eg = T02_Rights_hxRecord_H_Data2Dw()
    logger = eg.logger
    logger.info("任务开始")
    stF = "%Y-%m-%d %H:%M:%S"
    # 默认为前一天
    ...
    try:
        ...
        F_or_N = eg.get_rowcount('T02_Rights_hxRecord_H')
        print(F_or_N)
        if F_or_N > 0:
