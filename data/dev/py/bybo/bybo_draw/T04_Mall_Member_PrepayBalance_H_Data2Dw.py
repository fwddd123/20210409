...
class T04_Mall_Member_PrepayBalance_H_Data2Dw(Bybo_base):
    def __init__(self):
        super().__init__()
...
if __name__ == '__main__':
    eg = T04_Mall_Member_PrepayBalance_H_Data2Dw()
    logger = eg.logger
    logger.info("任务开始")
    stF = "%Y-%m-%d %H:%M:%S"
    # 默认为前一天
    ...
    try:
        ...
        F_or_N = eg.get_rowcount('T04_Mall_Member_PrepayBalance_H')
        print(F_or_N)
        if F_or_N > 0:
