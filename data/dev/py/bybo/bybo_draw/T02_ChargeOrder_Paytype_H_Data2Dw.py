...
class T02_ChargeOrder_Paytype_H_Data2Dw(Bybo_base):
    def __init__(self):
        super().__init__()
        self.__name = 'T02_ChargeOrder_Paytype_H'
        self._taget_table = 'T02_ChargeOrder_Paytype_H'
    def Draw_data(self,end_date,begin_date):
        ...
        try:
            ...
            sql2Tmp = '''
                ...
                from (
                select 
                ifnull(concat(a.Region,LPAD(a.Id, 10, 0)),'') as ChargeOrderId
                ,ifnull(a.PayType,'') as PayType
                ,ifnull(a.ActualPrice1,'0') as ActualPrice
                ,ifnull(concat(a.Region,LPAD(OfficeId, 10, 0)),'') as OfficeId
                ,ifnull(a.Id,'') as Id
                ...
                from %s.chargeorder a
                where   PayType is not null or PayType  <>''
                union all
                select 
                ifnull(concat(a.Region,LPAD(a.Id, 10, 0)),'') as ChargeOrderId
                ,ifnull(a.PayType2,'') as PayType
                ,ifnull(a.ActualPrice2,'0') as ActualPrice
                ,ifnull(concat(a.Region,LPAD(OfficeId, 10, 0)),'') as OfficeId
                ,ifnull(a.Id,'') as Id
                ,ifnull(a.Region,'') as Region
                ...
                'bbkq_ods_ekanya.ChargeOrder'                                            as SourceName
                from %s.chargeorder a
                where   PayType2 is not null or PayType2  <>''
                union all
                select 
                ifnull(concat(a.Region,LPAD(a.Id, 10, 0)),'') as ChargeOrderId
                ,ifnull(a.PayType3,'') as PayType
                ,ifnull(a.ActualPrice3,'0') as ActualPrice
                ,ifnull(concat(a.Region,LPAD(OfficeId, 10, 0)),'') as OfficeId
                ,ifnull(a.Id,'') as Id
                ,ifnull(a.Region,'') as Region
                ,ifnull(a.IsInactive,'') as IsInactive
                ...
                'bbkq_ods_ekanya.ChargeOrder'                                            as SourceName
                from %s.chargeorder a
                where   PayType3 is not null or PayType3  <>'') a
                ...
                <=str_to_date(\'%s\','%%Y-%%m-%%d %%H:%%i:%%s')
                ''' % (self.__name,ods_ekanya,ods_ekanya,ods_ekanya,begin_date, end_date)
            print(sql2Tmp)
            cur.execute(sql2Tmp)
...
if __name__ == '__main__':
    ...
    eg = T02_ChargeOrder_Paytype_H_Data2Dw()
    logger = eg.logger
    logger.info("任务开始")
    stF = "%Y-%m-%d %H:%M:%S"
    # 默认为前一天
    ...
    try:
        ...
        F_or_N = eg.get_rowcount('T02_ChargeOrder_Paytype_H')
        print(F_or_N)
        if F_or_N > 0:
